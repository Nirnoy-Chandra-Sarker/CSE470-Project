...
def profcontact():
    try:
        if(request.method=='POST'):
            ...
            print(FullName, uniName, designation, mail, passw)
            entry = Profcontacts(FullName=FullName, mail = mail, passw=passw,uniName=uniName,designation=designation )
            db.session.add(entry)
            db.session.commit()
            print(entry)
...
def loginP():
    ...
    if(request.method== 'POST'):
        ...
        res = Profcontacts.query.filter(Profcontacts.mail==mail_in).first()
      
        print(type(res))
        if res.mail==mail_in and res.passw==passw_in:
            login_user(res[0])
            return redirect(url_for('userP'))
        elif res.mail==mail_in and res.passw!=passw_in:
            return 'password error' #need to add flash on top
